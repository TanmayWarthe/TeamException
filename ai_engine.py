...
if not _groq_api_key:
    try:
        ...
        _groq_api_key = (st.secrets.get("GROQ_API_KEY") or "").strip()
    except Exception:
        pass
...
if _groq_api_key:
    ...
    client = None
    print("[ai_engine] WARNING: GROQ_API_KEY not found. Checked:", _env_path)

# Primary model — fast and capable. Falls back to smaller model on rate-limit.
MODEL_PRIMARY = "llama-3.3-70b-versatile"
MODEL_FALLBACK = "llama3-8b-8192"


def _extract_json(raw: str) -> str:
    """Strip markdown fences around JSON regardless of position."""
    text = raw.strip()
    # Handle ```json ... ``` or ``` ... ``` wrappers
    if "```" in text:
        # Remove opening fence (may have language tag like ```json)
        parts = text.split("```")
        # parts[0] is before first ```, parts[1] is code block content, etc.
        for part in parts:
            stripped = part.strip()
...
def _chat(messages: list, temperature: float = 0.7, max_tokens: int = 2000,
          model: str = None) -> str:
    """Send a chat completion request with automatic model fallback and error handling."""
    if client is None:
        raise RuntimeError(
        ...
        try:
            ...
            if "quota" in error_msg.lower() or "429" in error_msg or "rate" in error_msg.lower():
                last_err = RuntimeError(
                    f"Groq rate limit hit on {m}. "
                    "Please wait a moment, or check usage at https://console.groq.com"
                )
                continue
...
Return ONLY valid JSON, no markdown."""
    ...
    try:
        ...
        return {
            ...
            "summary": "Unable to parse resume", "primary_domain": "Unknown",
            "years_of_experience": "Unknown", "strongest_skills": []
        }
...
Return ONLY valid JSON."""
        ...
        {"role": "user", "content": "Analyze the candidate's response now."}
    ]
...
Return ONLY valid JSON."""
        ...
        {"role": "user", "content": "Generate the HR interview questions."}
    ]
...
Return ONLY valid JSON."""
        ...
        {"role": "user", "content": "Analyze the candidate's response."}
    ]
    ...
    try:
        ...
        return {
            "communication_score": 5, "relevance_score": 5, "depth_score": 5,
            "confidence_level": "medium", "key_points_covered": [], "missing_points": [],
            "feedback": "Unable to fully analyze.", "strengths": [], "improvements": [],
            "follow_up_questions": []
        }
...
Return ONLY valid JSON."""
        ...
        {"role": "user", "content": "Generate the final report."}
    ]
    result = _chat(messages, temperature=0.3, max_tokens=2000)
    try:
        return json.loads(_extract_json(result))
        ...
        return {
            ...
            "communication_score": 50, "reasoning_score": 50,
            "problem_solving_score": 50,
            "executive_summary": "Interview completed. Unable to generate detailed report.",
            "detailed_feedback": {
                "technical_skills": "N/A", "problem_solving": "N/A", "communication": "N/A",
                "areas_of_strength": [], "areas_for_improvement": [],
...
# ── Voice DSA Agent ────────────────────────────────────────────────────────────
# NOTE: Defined ONCE here. Previous versions had this block duplicated 3 times,
# causing the last definition to silently overwrite the earlier ones.
